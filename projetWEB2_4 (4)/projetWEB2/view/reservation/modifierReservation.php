...
$stmt = $pdo->prepare("SELECT r.*, t.type_t FROM reservation r JOIN transport t ON r.id_t = t.id_t WHERE r.id_r = ?");
$stmt->execute([$id_r]);
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && !$isUsedElsewhere) {
    try {
        ...
        $stmt3 = $pdo->prepare("SELECT id_t FROM transport WHERE type_t = :type_t AND dispo = 1 AND etat = 1");
        ...
        foreach ($transports as $id_t) {
            ...
            $stmt4->execute(['id_t' => $id_t, 'id_r' => $id_r]);
